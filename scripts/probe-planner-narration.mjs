...
// narration lines that name the teacher, talk about the show, or drift out of Spanish — the planner must
// write first-person teaching in castellano only.
// Usage: node scripts/probe-planner-narration.mjs "topic"
import { readFileSync } from "node:fs";
const topic = process.argv[2] ?? "¿Cómo se calcula el ROAS de una campaña y cuándo engaña?";
...
const template = source.match(/return `(Diseña una clase de marketing[\s\S]*?sin preámbulo ni análisis\.)`;/)?.[1];
if (!template) throw new Error("planner template not found in lesson-producer.ts");
const system = source.match(/"(Eres un diseñador de currículo de marketing[^"]*)"/)?.[1];
const sceneCount = 12;
...
for (const [i, step] of plan.steps.entries()) {
  const selfReference = new RegExp(`\\b${teacherName}\\b`).test(step.narration)
    || /est[ae] (v[ií]deo|programa|canal|aula|clase)|\bhe (creado|hecho|generado)\b/i.test(step.narration);
  // The channel is Castilian: a narration line built out of English function words never reached Spanish.
  const looksEnglish = /\b(the|this|and|you can|we are|is a)\b/i.test(step.narration);
  const bad = selfReference || looksEnglish;
  if (bad) flagged += 1;
